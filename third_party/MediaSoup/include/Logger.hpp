...
#endif
//@Eric - add : if(Logger::channel != NULL)
#ifdef MS_LOG_TRACE
	#define MS_TRACE() \
		do \
		{ \
			...
			{ \
				if(Logger::channel != NULL){int loggerWritten = std::snprintf(Logger::buffer, Logger::bufferSize, "D(trace) " _MS_LOG_STR, _MS_LOG_ARG); \
...
#define MS_DEBUG_TAG(tag, desc, ...) \
	...
	{ \
		...
		{ \
			if(Logger::channel != NULL){int loggerWritten = std::snprintf(Logger::buffer, Logger::bufferSize, "D" _MS_LOG_STR_DESC desc, _MS_LOG_ARG, ##__VA_ARGS__); \
			Logger::channel->SendLog(Logger::buffer, static_cast<uint32_t>(loggerWritten));} \
		} \
	} \
	while (false)
...
#define MS_WARN_TAG(tag, desc, ...) \
	...
	{ \
		...
		{ \
			if(Logger::channel != NULL){int loggerWritten = std::snprintf(Logger::buffer, Logger::bufferSize, "W" _MS_LOG_STR_DESC desc, _MS_LOG_ARG, ##__VA_ARGS__); \
			Logger::channel->SendLog(Logger::buffer, static_cast<uint32_t>(loggerWritten));} \
		} \
	} \
	while (false)
...
#define MS_DUMP(desc, ...) \
	do \
	{ \
		if(Logger::channel != NULL){int loggerWritten = std::snprintf(Logger::buffer, Logger::bufferSize, "X" _MS_LOG_STR_DESC desc, _MS_LOG_ARG, ##__VA_ARGS__); \
		Logger::channel->SendLog(Logger::buffer, static_cast<uint32_t>(loggerWritten));} \
	} \
	while (false)
...
#define MS_DUMP_DATA(data, len) \
	do \
	{ \
		if(Logger::channel != NULL){int loggerWritten = std::snprintf(Logger::buffer, Logger::bufferSize, "X(data) " _MS_LOG_STR, _MS_LOG_ARG); \
		Logger::channel->SendLog(Logger::buffer, static_cast<uint32_t>(loggerWritten)); \
...
#define MS_ERROR(desc, ...) \
	...
	{ \
		...
		{ \
			if(Logger::channel != NULL){int loggerWritten = std::snprintf(Logger::buffer, Logger::bufferSize, "E" _MS_LOG_STR_DESC desc, _MS_LOG_ARG, ##__VA_ARGS__); \
			Logger::channel->SendLog(Logger::buffer, static_cast<uint32_t>(loggerWritten));} \
		} \
	} \
	while (false)
