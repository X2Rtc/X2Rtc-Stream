	...
	inline bool Producer::MangleRtpPacket(RTC::RtpPacket* packet, RTC::RtpStreamRecv* rtpStream) const
	{
		...
		// Mangle RTP header extensions.
		{
			...
			else if (this->kind == RTC::Media::Kind::VIDEO)
			{
				...
				//@Eric - add for simulcast
				// Proxy urn:ietf:params:rtp-hdrext:rid.
				extenValue = packet->GetExtension(this->rtpHeaderExtensionIds.rid, extenLen);
				...
				if (extenValue)
				{
					...
					//extensions.emplace_back(
					//	static_cast<uint8_t>(RTC::RtpHeaderExtensionUri::Type::RTP_STREAM_ID), extenLen, bufferPtr);
					extensions.emplace_back(static_cast<uint8_t>(this->rtpHeaderExtensionIds.rid), extenLen, bufferPtr);
