...
namespace RTC
{
	  ...
	  : RTC::Transport::Transport(shared, id, listener, data)
	{
		...
		try
		{
			...
			// Create a ICE server.
			//@Eric - 20230608 - can use self defined ice parameter
			std::string strIceUsername;
			std::string strIcePwd;
			if (data.find("Ice") != data.end()) {
				strIceUsername = data["Ice"]["uname"];
				strIcePwd = data["Ice"]["pwd"];
			}
			if (strIceUsername.empty()) {
				strIceUsername = Utils::Crypto::GetRandomString(32);
			}
			if (strIcePwd.empty()) {
				strIcePwd = Utils::Crypto::GetRandomString(32);
			}
			this->iceServer = new RTC::IceServer(
			  this, strIceUsername, strIcePwd);
			//end modify
