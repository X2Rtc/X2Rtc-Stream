...
namespace RTC
{
	...
	static constexpr uint8_t MaxNackRetries{20u};// { 10u };	//@Eric - 将最大重传次数从 10次 上调至 20次（还可以增大到40次
	static constexpr uint64_t TimerInterval{20u};// { 40u };	//@Eric - 将定时器间隔从 40ms/次 下调到 20ms/次；
	...
	bool NackGenerator::ReceivePacket(RTC::RtpPacket* packet, bool isRecovered)
	{
		...
		if (isRecovered)
		{
			//this->recoveredList.insert(seq);	//@Eric - 20230805 - 这里如果存了这个seq在recoveredList，实际返回false，rtx包并没有回调给业务，而Nack逻辑：由于是recoveredList的包，又不发送nack，导致丢包后无法恢复
	...
	std::vector<uint16_t> NackGenerator::GetNackBatch(NackFilter filter)
	{
		...
		while (it != this->nackList.end())
		{
			...
			if (filter == NackFilter::TIME && (nackInfo.sentAtMs == 0 || nowMs - nackInfo.sentAtMs >= limit_var))
				// =================== nack test ===================
