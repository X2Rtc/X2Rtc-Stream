...
namespace mediasoupclient
{
	namespace Sdp
	{
		MediaSection::MediaSection(const json& iceParameters, const json& iceCandidates)
		{
			...
			if (this->mediaObject["candidates"].size() > 0) {
				this->mediaObject["endOfCandidates"] = "end-of-candidates";
			}
			this->mediaObject["iceOptions"] = "trickle"; //@Eric - "renomination";
		}
		...
		json MediaSection::GetJObject() const
		{
			MS_TRACE();
		  ...
		  : MediaSection(iceParameters, iceCandidates)
		{
			...
			//@Eric - NOTE: We take the latest fingerprint.
			json jsFingerprints = dtlsParameters["fingerprints"];
			for (json::iterator fingerprint_it = jsFingerprints.begin(); fingerprint_it != jsFingerprints.end(); ++fingerprint_it) {
				if ((*fingerprint_it)["algorithm"] == "sha-256") {
					this->mediaObject["fingerprint"] = {
				{ "type", (*fingerprint_it)["algorithm"] },
				{ "hash", (*fingerprint_it)["value"] }
					};
					break;
				}
			}
