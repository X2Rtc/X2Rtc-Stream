...
static unsigned int GetJsonUint(rapidjson::Document&jsDoc, const char*strKey, const char* location)
{
	if (jsDoc.HasMember(strKey) && jsDoc[strKey].IsUint()) {
		return jsDoc[strKey].GetUint();
	}
	else {
		if (gJsonGetErrorEvent != NULL) {
			char strLog[512];
			sprintf(strLog, "Get uint nil, key is: %s at: %s", strKey, location);
			gJsonGetErrorEvent(strLog);
		}
	}
	return 0;
...
{
	if (jsDoc.HasMember(strKey) && (jsDoc[strKey].IsInt64() || jsDoc[strKey].IsUint64())) {
		if (jsDoc[strKey].IsInt64())
			return jsDoc[strKey].GetInt64();
		else
			return jsDoc[strKey].GetUint64();
	}
	else {
		if (gJsonGetErrorEvent != NULL) {
			char strLog[512];
			sprintf(strLog, "Get int64 nil, key is: %s at: %s", strKey, location);
			gJsonGetErrorEvent(strLog);
		}
	}
	return -1;
...
//====
static bool HasJsonStr(rapidjson::Value&jsDoc, const char*strKey)
{
	if (jsDoc.HasMember(strKey) && jsDoc[strKey].IsString()) {
		return true;
	}
...
static int HasJsonInt(rapidjson::Value&jsDoc, const char*strKey)
{
	if (jsDoc.HasMember(strKey) && jsDoc[strKey].IsInt()) {
		return true;
	}
...
static int HasJsonBool(rapidjson::Value&jsDoc, const char*strKey)
{
	if (jsDoc.HasMember(strKey) && jsDoc[strKey].IsBool()) {
		return true;
	}
...
static const char* GetJsonStr(rapidjson::Value&jsDoc, const char*strKey, const char* location)
{
	if (jsDoc.HasMember(strKey) && jsDoc[strKey].IsString()) {
		return jsDoc[strKey].GetString();
	}
...
static int GetJsonInt(rapidjson::Value&jsDoc, const char*strKey, const char* location)
{
	if (jsDoc.HasMember(strKey) && (jsDoc[strKey].IsInt() || jsDoc[strKey].IsUint())) {
		if (jsDoc[strKey].IsInt())
			return jsDoc[strKey].GetInt();
...
static unsigned int GetJsonUint(rapidjson::Value&jsDoc, const char*strKey, const char* location)
{
	if (jsDoc.HasMember(strKey) && (jsDoc[strKey].IsInt() || jsDoc[strKey].IsUint())) {
		if (jsDoc[strKey].IsInt())
			return jsDoc[strKey].GetInt();
...
static int64_t GetJsonInt64(rapidjson::Value&jsDoc, const char*strKey, const char* location)
{
	if (jsDoc.HasMember(strKey) && (jsDoc[strKey].IsInt64() || jsDoc[strKey].IsUint64())) {
		if (jsDoc[strKey].IsInt64())
			return jsDoc[strKey].GetInt64();
...
static bool GetJsonBool(rapidjson::Value&jsDoc, const char*strKey, const char* location)
{
	if (jsDoc.HasMember(strKey) && jsDoc[strKey].IsBool()) {
		return jsDoc[strKey].GetBool();
	}
