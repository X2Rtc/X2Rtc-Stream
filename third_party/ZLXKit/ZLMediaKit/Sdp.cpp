...
static onceToken token([]() {
    mINI::Instance()[kPreferredCodecA] = "opus";// "PCMU,PCMA,opus,mpeg4-generic";
    mINI::Instance()[kPreferredCodecV] = "H264";// "H264,H265,AV1X,VP9,VP8";
});
}
...
RtcSessionSdp::Ptr RtcSession::toRtcSessionSdp() const{
    ...
    if (isIceLite()) {
        sdp.addAttr(std::make_shared<SdpCommon>("ice-lite"));   //@Eric - ice-lite 要加在media的外面，因为MediaSoup不主动发Stun binding。如果没有ice-lite webrtc页面一直以ice:Controlled身份发送请求
    }
    for (auto &m : media) {
...
bool RtcSession::isIceLite() const
{
    for (auto& m : media) {
        if (m.ice_lite) {
            return true;
...
void RtcConfigure::RtcTrackConfigure::clearAll()
{
    rtcp_mux = false;
    rtcp_rsize = false;
    group_bundle = false;
...
void RtcConfigure::createOffer(const RtcSession &offer) const
{
