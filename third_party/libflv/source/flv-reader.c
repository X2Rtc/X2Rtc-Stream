...
static int flv_read_header(struct flv_reader_t* flv)
{
	...
	RTC_F_CHECK(h.offset >= FLV_HEADER_SIZE && h.offset < FLV_HEADER_SIZE + 4096);
...
int flv_reader_read(void* p, int* tagtype, uint32_t* timestamp, size_t* taglen, void* buffer, size_t bytes)
{
	...
	*timestamp = tag.timestamp;
    ...
    RTC_F_CHECK(0 == tag.streamId); // StreamID Always 0
    RTC_F_CHECK(sz == tag.size + FLV_TAG_HEADER_SIZE);
