...
{
	uint8_t i;
	RTC_F_CHECK(ptr >= hevc->data && ptr + bytes <= end && end <= hevc->data + sizeof(hevc->data));
...
{
	// copy new
	RTC_F_CHECK(hevc->numOfArrays < sizeof(hevc->nalu) / sizeof(hevc->nalu[0]));
...
void hevc_annexbtomp4_test(void)
{
	...
	RTC_F_CHECK(h265_annexbtomp4(&hevc, annexb, sizeof(annexb), output, sizeof(output), &vcl, &update) > 0);
	RTC_F_CHECK(3 == hevc.numOfArrays && vcl == 0 && update == 1);
	RTC_F_CHECK(hevc.nalu[0].bytes == sizeof(vps) && 0 == memcmp(hevc.nalu[0].data, vps, sizeof(vps)));
	RTC_F_CHECK(hevc.nalu[1].bytes == sizeof(sps) && 0 == memcmp(hevc.nalu[1].data, sps, sizeof(sps)));
	RTC_F_CHECK(hevc.nalu[2].bytes == sizeof(pps) && 0 == memcmp(hevc.nalu[2].data, pps, sizeof(pps)));
}
#endif
