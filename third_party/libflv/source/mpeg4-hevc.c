...
static int _mpeg4_hevc_decoder_configuration_record_load(const uint8_t* data, size_t bytes, struct mpeg4_hevc_t* hevc)
{
	...
	for (i = 0; i < numOfArrays; i++)
	{
		...
		for (j = 0; j < n; j++)
		{
			...
			RTC_F_CHECK((nalutype & 0x3F) == ((p[2] >> 1) & 0x3F));
...
int mpeg4_hevc_decoder_configuration_record_save(const struct mpeg4_hevc_t* hevc, uint8_t* data, size_t bytes)
{
	...
	RTC_F_CHECK(hevc->lengthSizeMinusOne <= 3);
	end = data + bytes;
	if (bytes < 23)
	...
	// 8.3.3.1.2 Syntax
	RTC_F_CHECK(1 == hevc->configurationVersion);
...
//	data[22] = hevc->numOfArrays;
	...
	for (k = i = 0; i < sizeof(nalu)/sizeof(nalu[0]); i++)
	{
		...
		{
			RTC_F_CHECK(hevc->nalu[j].type == ((hevc->nalu[j].data[0] >> 1) & 0x3F));
			...
			RTC_F_CHECK(hevc->nalu[i].data + hevc->nalu[j].bytes <= hevc->data + sizeof(hevc->data));
...
int mpeg4_hevc_to_nalu(const struct mpeg4_hevc_t* hevc, uint8_t* data, size_t bytes)
{
	...
	for (i = 0; i < hevc->numOfArrays; i++)
	{
		...
		RTC_F_CHECK(hevc->nalu[i].type == ((hevc->nalu[i].data[0] >> 1) & 0x3F));
...
void mpeg4_hevc_test(void)
{
	...
	RTC_F_CHECK(sizeof(src) == mpeg4_hevc_decoder_configuration_record_load(src, sizeof(src), &hevc));
	RTC_F_CHECK(0 == hevc.general_profile_space && 0 == hevc.general_tier_flag);
	RTC_F_CHECK(1 == hevc.general_profile_idc && 0xb4 == hevc.general_level_idc);
	RTC_F_CHECK(1 == hevc.numTemporalLayers && 1 == hevc.temporalIdNested);
	RTC_F_CHECK(3 == hevc.numOfArrays);
	RTC_F_CHECK(sizeof(src) == mpeg4_hevc_decoder_configuration_record_save(&hevc, data, sizeof(data)));
	RTC_F_CHECK(0 == memcmp(src, data, sizeof(src)));
    mpeg4_hevc_codecs(&hevc, (char*)data, sizeof(data));
    RTC_F_CHECK(0 == memcmp("hvc1.1.6.L180", data, 13));

	RTC_F_CHECK(sizeof(nalu) == mpeg4_hevc_to_nalu(&hevc, data, sizeof(data)));
	RTC_F_CHECK(0 == memcmp(nalu, data, sizeof(nalu)));
