...
int webm_vpx_codec_configuration_record_load(const uint8_t* data, size_t bytes, struct webm_vpx_t* vpx)
{
    ...
    RTC_F_CHECK(0 == vpx->codec_intialization_data_size);
    return 8;
}
...
void webm_vpx_test(void)
{
    ...
    RTC_F_CHECK(sizeof(src) == webm_vpx_codec_configuration_record_load(src, sizeof(src), &vpx));
    RTC_F_CHECK(0 == vpx.profile && 31 == vpx.level && 8 == vpx.bit_depth && 0 == vpx.chroma_subsampling && 0 == vpx.video_full_range_flag);
    RTC_F_CHECK(sizeof(src) == webm_vpx_codec_configuration_record_save(&vpx, data, sizeof(data)));
