...
int mpeg4_aac_adts_load(const uint8_t* data, size_t bytes, struct mpeg4_aac_t* aac)
{
	...
	RTC_F_CHECK(0xFF == data[0] && 0xF0 == (data[1] & 0xF0)); /* syncword */
	...
	RTC_F_CHECK(aac->profile > 0 && aac->profile < 31);
	RTC_F_CHECK(aac->channel_configuration >= 0 && aac->channel_configuration <= 7);
	RTC_F_CHECK(aac->sampling_frequency_index >= 0 && aac->sampling_frequency_index <= 0xc);
...
int mpeg4_aac_stream_mux_config_load(const uint8_t* data, size_t bytes, struct mpeg4_aac_t* aac)
{
	...
	if (6 == bytes && 0x40 == data[0] && 0 == (data[1] & 0xFE))
	{
		...
		RTC_F_CHECK(0 == (0x80 & data[0])); // audioMuxVersion: 0
...
//	const unsigned char ascsbr[] = { 0x13, 0x10, 0x56, 0xe5, 0x9d, 0x48, 0x00 };
	...
	RTC_F_CHECK(sizeof(ascsbr) == mpeg4_aac_audio_specific_config_load(ascsbr, sizeof(ascsbr), &aac));
	RTC_F_CHECK(2 == aac.profile && 7 == aac.sampling_frequency_index && 2 == aac.channel_configuration);
	//RTC_F_CHECK(sizeof(ascsbr) == mpeg4_aac_audio_specific_config_save(&aac, data, sizeof(data)));
	//RTC_F_CHECK(0 == memcmp(ascsbr, data, sizeof(ascsbr)));

	RTC_F_CHECK(sizeof(asc) == mpeg4_aac_audio_specific_config_load(asc, sizeof(asc), &aac));
	RTC_F_CHECK(2 == aac.profile && 7 == aac.sampling_frequency_index && 1 == aac.channel_configuration);
	RTC_F_CHECK(sizeof(asc) == mpeg4_aac_audio_specific_config_save(&aac, data, sizeof(data)));
	RTC_F_CHECK(0 == memcmp(asc, data, sizeof(asc)));

	RTC_F_CHECK(sizeof(adts) == mpeg4_aac_adts_save(&aac, 1, data, sizeof(data)));
	RTC_F_CHECK(0 == memcmp(adts, data, sizeof(adts)));
	RTC_F_CHECK(7 == mpeg4_aac_adts_load(data, sizeof(adts), &aac2));
	RTC_F_CHECK(0 == memcmp(&aac, &aac2, sizeof(aac)));

	RTC_F_CHECK(22050 == mpeg4_aac_audio_frequency_to(aac.sampling_frequency_index));
	RTC_F_CHECK(aac.sampling_frequency_index == mpeg4_aac_audio_frequency_from(22050));

	//RTC_F_CHECK(sizeof(ascsbr) == mpeg4_aac_audio_specific_config_load(ascsbr, sizeof(ascsbr), &aac));
	//RTC_F_CHECK(2 == aac.profile && 6 == aac.sampling_frequency_index && 1 == aac.channel_configuration);

	RTC_F_CHECK(sizeof(asc8ch) == mpeg4_aac_audio_specific_config_load(asc8ch, sizeof(asc8ch), &aac));
	RTC_F_CHECK(2 == aac.profile && 4 == aac.sampling_frequency_index && 8 == aac.channels);
	RTC_F_CHECK(29 == mpeg4_aac_adts_save(&aac, 1, data, sizeof(data)));
	...
	//RTC_F_CHECK(0 == memcmp(data, mux1, sizeof(mux1)));
}
#endif
