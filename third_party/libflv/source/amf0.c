...
#define AMF_OBJECT_ITEM_VALUE(v, amf_type, amf_name, amf_value, amf_size) { v.type=amf_type; v.name=amf_name; v.value=amf_value; v.size=amf_size; }
	...
	RTC_F_CHECK(end == amf_read_items(amf0, end, items, sizeof(items) / sizeof(items[0])));
	RTC_F_CHECK(0 == strcmp(fms.fmsVer, "FMS/3,5,5,2004"));
	RTC_F_CHECK(fms.capabilities == 31.0);
	RTC_F_CHECK(fms.mode == 1.0);
	RTC_F_CHECK(0 == strcmp(result.code, "NetConnection.Connect.Success"));
	RTC_F_CHECK(0 == strcmp(result.level, "status"));
	RTC_F_CHECK(0 == strcmp(result.description, "Connection succeeded."));
	RTC_F_CHECK(0 == strcmp(result.data.version, "3,5,5,2004"));
	RTC_F_CHECK(1584259571.0 == result.clientid);
	RTC_F_CHECK(3.0 == result.objectEncoding);
}
...
#define AMF_OBJECT_ITEM_VALUE(v, amf_type, amf_name, amf_value, amf_size) { v.type=amf_type; v.name=amf_name; v.value=amf_value; v.size=amf_size; }
	...
	RTC_F_CHECK(end == amf_read_items(amf0, end, items, sizeof(items) / sizeof(items[0])));
	RTC_F_CHECK(0 == strcmp(connect.app, "live/"));
	RTC_F_CHECK(0 == strcmp(connect.tcUrl, "rtmp://push.rtmp.com/live/"));
	RTC_F_CHECK(0 == strcmp(connect.flashver, "FMLE/3.0 (compatible; FMSc/1.0)"));
	RTC_F_CHECK(0 == strcmp(connect.swfUrl, "rtmp://push.rtmp.com/live/"));
	RTC_F_CHECK(0 == strcmp(connect.pageUrl, "")); // pageUrl undefined
	RTC_F_CHECK(connect.fpad == 0);
	RTC_F_CHECK(connect.capabilities == 15);
	RTC_F_CHECK(connect.audioCodecs == 3191);
	RTC_F_CHECK(connect.videoCodecs == 252);
	RTC_F_CHECK(connect.videoFunction == 1);
	RTC_F_CHECK(connect.encoding == 0);
}
