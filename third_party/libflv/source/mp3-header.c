...
int mp3_get_frequency(const struct mp3_header_t* mp3)
{
	...
	default: RTC_F_CHECK(0); return -1;
	}
}
...
void mp3_header_test(void)
{
	...
	RTC_F_CHECK(4 == mp3_header_load(&mp3, v, 4));
	RTC_F_CHECK(MP3_MPEG1 == mp3.version && MP3_LAYER3 == mp3.layer);
	RTC_F_CHECK(14 == mp3.bitrate_index && 320000 == mp3_get_bitrate(&mp3));
	RTC_F_CHECK(0 == mp3.sampling_frequency && 44100 == mp3_get_frequency(&mp3));
	RTC_F_CHECK(1 == mp3.mode && 1 == mp3.protection);
	RTC_F_CHECK(4 == mp3_header_save(&mp3, v2, 4));
	RTC_F_CHECK(0 ==  memcmp(v, v2, 4));
}
#endif
