...
static int mpeg4_aac_pce_load(struct mpeg4_bits_t* bits, struct mpeg4_aac_t* aac, struct mpeg4_bits_t* pce)
{
	...
	RTC_F_CHECK(aac->sampling_frequency_index == sampling_frequency_index);
	RTC_F_CHECK(aac->profile == object_type + 1);
...
//	RTC_F_CHECK(data[1] & 0x01); // disable protection_absent
	...
	RTC_F_CHECK(src.channels == aac->channels && (adts.bits + 7) / 8 <= bytes);
