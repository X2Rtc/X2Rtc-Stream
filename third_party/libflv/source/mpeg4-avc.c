...
static int _mpeg4_avc_decoder_configuration_record_load(const uint8_t* data, size_t bytes, struct mpeg4_avc_t* avc)
{
	...
	if (bytes < 7) return -1;
	//RTC_F_CHECK(1 == data[0]);	//@Eric - remove
//	avc->version = data[0];
	avc->profile = data[1];
...
int mpeg4_avc_decoder_configuration_record_save(const struct mpeg4_avc_t* avc, uint8_t* data, size_t bytes)
{
	...
	RTC_F_CHECK(0 < avc->nalu && avc->nalu <= 4);
...
void mpeg4_avc_test(void)
{
	...
	RTC_F_CHECK(sizeof(src) == mpeg4_avc_decoder_configuration_record_load(src, sizeof(src), &avc));
	RTC_F_CHECK(0x42 == avc.profile && 0xe0 == avc.compatibility && 0x1e == avc.level);
	RTC_F_CHECK(4 == avc.nalu && 1 == avc.nb_sps && 1 == avc.nb_pps);
	RTC_F_CHECK(sizeof(src) == mpeg4_avc_decoder_configuration_record_save(&avc, data, sizeof(data)));
	RTC_F_CHECK(0 == memcmp(src, data, sizeof(src)));
    mpeg4_avc_codecs(&avc, (char*)data, sizeof(data));
    RTC_F_CHECK(0 == memcmp("avc1.42e01e", data, 11));

	RTC_F_CHECK(sizeof(nalu) == mpeg4_avc_to_nalu(&avc, data, sizeof(data)));
