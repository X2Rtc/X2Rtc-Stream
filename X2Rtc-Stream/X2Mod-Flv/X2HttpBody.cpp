...
X2HttpStringBody::X2HttpStringBody(string str) {
    _str = std::move(str);
}

int64_t X2HttpStringBody::remainSize() {
    return _str.size() - _offset;
}

Buffer::Ptr X2HttpStringBody::readData(size_t size) {
...
X2HttpFileBody::X2HttpFileBody(const string &file_path, bool use_mmap) {
#ifdef ENABLE_MMAP
    if (use_mmap ) {
...
void X2HttpFileBody::setRange(uint64_t offset, uint64_t max_size) {
...
int X2HttpFileBody::sendFile(int fd) {
#if defined(__linux__) || defined(__linux)
    if (!_fp) {
        return -1;
    }
...
int64_t X2HttpFileBody::remainSize() {
    return _read_to - _file_offset;
}

Buffer::Ptr X2HttpFileBody::readData(size_t size) {
...
X2HttpMultiFormBody::X2HttpMultiFormBody(const X2HttpArgs &args, const string &filePath, const string &boundary) {
    _fileBody = std::make_shared<X2HttpFileBody>(filePath);
...
int64_t X2HttpMultiFormBody::remainSize() {
    return _totalSize - _offset;
}

Buffer::Ptr X2HttpMultiFormBody::readData(size_t size) {
    if (_bodyPrefix.size()) {
...
string X2HttpMultiFormBody::multiFormBodySuffix(const string &boundary) {
    return "\r\n--" + boundary + "--";
}

string X2HttpMultiFormBody::multiFormContentType(const string &boundary) {
    return StrPrinter << "multipart/form-data; boundary=" << boundary;
}

string X2HttpMultiFormBody::multiFormBodyPrefix(const X2HttpArgs &args, const string &boundary, const string &fileName) {
...
X2HttpBufferBody::X2HttpBufferBody(Buffer::Ptr buffer) {
    _buffer = std::move(buffer);
}

int64_t X2HttpBufferBody::remainSize() {
    return _buffer ? _buffer->size() : 0;
}

Buffer::Ptr X2HttpBufferBody::readData(size_t size) {
