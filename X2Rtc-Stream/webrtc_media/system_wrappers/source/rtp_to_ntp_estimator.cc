  ...
  *new_rtcp_sr = false;
  ...
  if (!measurements_.empty()) {
    ...
    if (ntp_ms_new <= old_ntp_ms ||
      ...
      //RTC_LOG(LS_WARNING) << "Newer RTCP SR report with older RTP timestamp, dropping";
      invalid_sample = true;
  ...
  if (invalid_sample) {
    ...
    //RTC_LOG(LS_WARNING) << "Multiple consecutively invalid RTCP SR reports, clearing measurements.";
    measurements_.clear();
