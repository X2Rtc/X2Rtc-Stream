#define MS_CLASS "RTC::X2PortManager"
...
namespace x2rtc
{
	...
	thread_local absl::flat_hash_map<std::string, std::vector<bool>> X2PortManager::mapUdpIpPorts;
	thread_local absl::flat_hash_map<std::string, std::vector<bool>> X2PortManager::mapTcpIpPorts;

	/* Class methods. */

	uv_handle_t* X2PortManager::Bind(Transport transport, std::string& ip)
	{
		MS_TRACE();

		// First normalize the IP. This may throw if invalid IP.
		X2Utils::IP::NormalizeIp(ip);

		int err;
		const int family = X2Utils::IP::GetFamily(ip);
		struct sockaddr_storage bindAddr; // NOLINT(cppcoreguidelines-pro-type-member-init)
		size_t portIdx;
		int flags{ 0 };
		std::vector<bool>& ports = X2PortManager::GetPorts(transport, ip);
		size_t attempt{ 0u };
		...
		portIdx = static_cast<size_t>(X2Utils::Crypto::GetRandomUInt(static_cast<uint32_t>(0), static_cast<uint32_t>(ports.size() - 1)));
		...
		while (true)
		{
			...
			port = static_cast<uint16_t>(portIdx + X2Settings::configuration.rtcMinPort);
			...
			switch (transport)
			{
				case Transport::UDP:
					...
					err      = uv_udp_init_ex(
						uv_default_loop(), reinterpret_cast<uv_udp_t*>(uvHandle), UV_UDP_RECVMMSG);
					break;

				case Transport::TCP:
					uvHandle = reinterpret_cast<uv_handle_t*>(new uv_tcp_t());
					err      = uv_tcp_init(uv_default_loop(), reinterpret_cast<uv_tcp_t*>(uvHandle));
					break;
			}
	...
	uv_handle_t* X2PortManager::Bind(Transport transport, std::string& ip, uint16_t port)
	{
		MS_TRACE();
	...
	void X2PortManager::Unbind(Transport transport, std::string& ip, uint16_t port)
	{
		MS_TRACE();

		if (
		  (static_cast<size_t>(port) < X2Settings::configuration.rtcMinPort) ||
		  (static_cast<size_t>(port) > X2Settings::configuration.rtcMaxPort))
		{
		...
		const size_t portIdx = static_cast<size_t>(port) - X2Settings::configuration.rtcMinPort;

		switch (transport)
		{
			case Transport::UDP:
			{
				auto it = X2PortManager::mapUdpIpPorts.find(ip);

				if (it == X2PortManager::mapUdpIpPorts.end())
					return;
			...
			case Transport::TCP:
			{
				auto it = X2PortManager::mapTcpIpPorts.find(ip);

				if (it == X2PortManager::mapTcpIpPorts.end())
					return;
	...
	std::vector<bool>& X2PortManager::GetPorts(Transport transport, const std::string& ip)
	{
		MS_TRACE();
		...
		switch (transport)
		{
			case Transport::UDP:
			{
				...
				const uint16_t numPorts =
				  X2Settings::configuration.rtcMaxPort - X2Settings::configuration.rtcMinPort + 1;

				// Emplace a new vector filled with numPorts false values, meaning that
				// all ports are available.
				auto pair = X2PortManager::mapUdpIpPorts.emplace(
			...
			case Transport::TCP:
			{
				...
				// Emplace a new vector filled with numPorts false values, meaning that
				// all ports are available.
				auto pair = X2PortManager::mapTcpIpPorts.emplace(
				  std::piecewise_construct, std::make_tuple(ip), std::make_tuple(numPorts, false));
