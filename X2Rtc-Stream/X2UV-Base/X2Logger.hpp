 ...
 * source file including X2Logger.hpp MUST define its own MS_CLASS macro. Include
 * files (.hpp) MUST NOT include X2Logger.hpp.
 *
...
#ifndef X2_MS_LOGGER_HPP
#define X2_MS_LOGGER_HPP

#include "X2common.hpp"
#include "X2LogLevel.hpp"
#include "X2Settings.hpp"
...
#define _MS_TAG_ENABLED(tag) X2Settings::configuration.logTags.tag
#define _MS_TAG_ENABLED_2(tag1, tag2) (X2Settings::configuration.logTags.tag1 || X2Settings::configuration.logTags.tag2)
...
public:
	X2LogCallback() {};
	virtual ~X2LogCallback() {};

	virtual void SendLog(char* message, uint32_t messageLen) {
	};
};
...
{
public:
	static void ClassInit(X2LogCallback* channel);

public:
	static const uint64_t pid;
	thread_local static X2LogCallback* channel;
...
#endif
//@Eric - add : if(X2Logger::channel != NULL)
#ifdef MS_LOG_TRACE
	#define MS_TRACE() \
		do \
		{ \
			if (X2Settings::configuration.logLevel == LogLevel::LOG_DEBUG) \
			{ \
				if(X2Logger::channel != NULL){int loggerWritten = std::snprintf(X2Logger::buffer, X2Logger::bufferSize, "D(trace) " _MS_LOG_STR, _MS_LOG_ARG); \
				X2Logger::channel->SendLog(X2Logger::buffer, static_cast<uint32_t>(loggerWritten));} \
			} \
		} \
		while (false)
...
#define MS_HAS_DEBUG_TAG(tag) \
	(X2Settings::configuration.logLevel == LogLevel::LOG_DEBUG && _MS_TAG_ENABLED(tag))

#define MS_HAS_WARN_TAG(tag) \
	(X2Settings::configuration.logLevel >= LogLevel::LOG_WARN && _MS_TAG_ENABLED(tag))

#define MS_DEBUG_TAG(tag, desc, ...) \
	do \
	{ \
		if (X2Settings::configuration.logLevel == LogLevel::LOG_DEBUG && _MS_TAG_ENABLED(tag)) \
		{ \
			if(X2Logger::channel != NULL){int loggerWritten = std::snprintf(X2Logger::buffer, X2Logger::bufferSize, "D" _MS_LOG_STR_DESC desc, _MS_LOG_ARG, ##__VA_ARGS__); \
			X2Logger::channel->SendLog(X2Logger::buffer, static_cast<uint32_t>(loggerWritten));} \
		} \
	} \
	while (false)
...
#define MS_WARN_TAG(tag, desc, ...) \
	do \
	{ \
		if (X2Settings::configuration.logLevel >= LogLevel::LOG_WARN && _MS_TAG_ENABLED(tag)) \
		{ \
			if(X2Logger::channel != NULL){int loggerWritten = std::snprintf(X2Logger::buffer, X2Logger::bufferSize, "W" _MS_LOG_STR_DESC desc, _MS_LOG_ARG, ##__VA_ARGS__); \
			X2Logger::channel->SendLog(X2Logger::buffer, static_cast<uint32_t>(loggerWritten));} \
		} \
	} \
	while (false)
...
#define MS_DEBUG_2TAGS(tag1, tag2, desc, ...) \
	do \
	{ \
		if (X2Settings::configuration.logLevel == LogLevel::LOG_DEBUG && _MS_TAG_ENABLED_2(tag1, tag2)) \
		{ \
			if(X2Logger::channel != NULL){int loggerWritten = std::snprintf(X2Logger::buffer, X2Logger::bufferSize, "D" _MS_LOG_STR_DESC desc, _MS_LOG_ARG, ##__VA_ARGS__); \
			X2Logger::channel->SendLog(X2Logger::buffer, static_cast<uint32_t>(loggerWritten));} \
		} \
...
#define MS_WARN_2TAGS(tag1, tag2, desc, ...) \
	do \
	{ \
		if (X2Settings::configuration.logLevel >= LogLevel::LOG_WARN && _MS_TAG_ENABLED_2(tag1, tag2)) \
		{ \
			if(X2Logger::channel != NULL){int loggerWritten = std::snprintf(X2Logger::buffer, X2Logger::bufferSize, "W" _MS_LOG_STR_DESC desc, _MS_LOG_ARG, ##__VA_ARGS__); \
			X2Logger::channel->SendLog(X2Logger::buffer, static_cast<uint32_t>(loggerWritten));} \
		} \
...
#define MS_DUMP(desc, ...) \
	do \
	{ \
		if(X2Logger::channel != NULL){int loggerWritten = std::snprintf(X2Logger::buffer, X2Logger::bufferSize, "X" _MS_LOG_STR_DESC desc, _MS_LOG_ARG, ##__VA_ARGS__); \
		X2Logger::channel->SendLog(X2Logger::buffer, static_cast<uint32_t>(loggerWritten));} \
	} \
	while (false)
...
#define MS_DUMP_DATA(data, len) \
	do \
	{ \
		if(X2Logger::channel != NULL){int loggerWritten = std::snprintf(X2Logger::buffer, X2Logger::bufferSize, "X(data) " _MS_LOG_STR, _MS_LOG_ARG); \
		X2Logger::channel->SendLog(X2Logger::buffer, static_cast<uint32_t>(loggerWritten)); \
		...
		{ \
		  ...
		  { \
		  	if (bufferDataLen != 0) \
		  	{ \
		  		X2Logger::channel->SendLog(X2Logger::buffer, static_cast<uint32_t>(bufferDataLen)); \
		  		bufferDataLen = 0; \
		  	} \
		    int loggerWritten = std::snprintf(X2Logger::buffer + bufferDataLen, X2Logger::bufferSize, "X%06X ", static_cast<unsigned int>(i)); \
		    bufferDataLen += loggerWritten; \
		  } \
		  int loggerWritten = std::snprintf(X2Logger::buffer + bufferDataLen, X2Logger::bufferSize, "%02X ", static_cast<unsigned char>(data[i])); \
		  bufferDataLen += loggerWritten; \
		} \
		if (bufferDataLen != 0) \
			X2Logger::channel->SendLog(X2Logger::buffer, static_cast<uint32_t>(bufferDataLen));} \
	...
	{ \
		...
		{ \
		  ...
		  { \
		  	if (bufferDataLen != 0) \
		  	{ \
		  		X2Logger::buffer[bufferDataLen] = '\0'; \
		  		std::fprintf(stdout, "%s", X2Logger::buffer); \
		  		bufferDataLen = 0; \
		  	} \
		    int loggerWritten = std::snprintf(X2Logger::buffer + bufferDataLen, X2Logger::bufferSize, "\n%06X ", static_cast<unsigned int>(i)); \
		    bufferDataLen += loggerWritten; \
		  } \
		  int loggerWritten = std::snprintf(X2Logger::buffer + bufferDataLen, X2Logger::bufferSize, "%02X ", static_cast<unsigned char>(data[i])); \
		  bufferDataLen += loggerWritten; \
...
#define MS_ERROR(desc, ...) \
	do \
	{ \
		if (X2Settings::configuration.logLevel >= LogLevel::LOG_ERROR || MS_LOG_DEV_LEVEL >= 1) \
		{ \
			if(X2Logger::channel != NULL){int loggerWritten = std::snprintf(X2Logger::buffer, X2Logger::bufferSize, "E" _MS_LOG_STR_DESC desc, _MS_LOG_ARG, ##__VA_ARGS__); \
			X2Logger::channel->SendLog(X2Logger::buffer, static_cast<uint32_t>(loggerWritten));} \
		} \
	} \
	while (false)
