...
public:
	...
	struct UvSendData
	{
		...
		X2UdpSocketHandler::onSendCallback* cb{ nullptr };
	};
...
public:
	 ...
	 */
	explicit X2UdpSocketHandler(uv_udp_t* uvHandle);
	X2UdpSocketHandler& operator=(const X2UdpSocketHandler&) = delete;
	X2UdpSocketHandler(const X2UdpSocketHandler&)            = delete;
	virtual ~X2UdpSocketHandler();
...
public:
	//@Eric - 20230808 - Add for allow setting udp send/receive buffer size
	void SetSendBufferSize(int nBufSize) { if (uvHandle != NULL) uv_send_buffer_size((uv_handle_t*)uvHandle, &nBufSize); };
	void SetRecvBufferSize(int nBufSize) { if (uvHandle != NULL) uv_recv_buffer_size((uv_handle_t*)uvHandle, &nBufSize); };
...
public:
	...
	void OnUvSend(int status, X2UdpSocketHandler::onSendCallback* cb);
