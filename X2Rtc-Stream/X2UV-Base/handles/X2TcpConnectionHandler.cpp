#define MS_CLASS "X2TcpConnectionHandler"
...
{
	auto* connection = static_cast<X2TcpConnectionHandler*>(handle->data);
...
{
	auto* writeData  = static_cast<X2TcpConnectionHandler::UvWriteData*>(req->data);
...
X2TcpConnectionHandler::X2TcpConnectionHandler(size_t bufferSize) : bufferSize(bufferSize)
{
	MS_TRACE();
...
X2TcpConnectionHandler::~X2TcpConnectionHandler()
{
	MS_TRACE();
...
void X2TcpConnectionHandler::Close()
{
	MS_TRACE();
	...
	// Tell the UV handle that the X2TcpConnectionHandler has been closed.
	...
	if (err != 0)
		MS_THROW_ERROR("uv_read_stop() failed: %s", uv_strerror(err));
	...
	if (!this->hasError && !this->isClosedByPeer)
	{
      ...
      req, reinterpret_cast<uv_stream_t*>(this->uvHandle), static_cast<uv_shutdown_cb>(onShutdown));
		...
		if (err != 0)
		{
			...
			this->uvHandle = NULL;
			MS_THROW_ERROR("uv_shutdown() failed: %s", uv_strerror(err));
		}
	}
...
void X2TcpConnectionHandler::Dump() const
{
	MS_DUMP("<X2TcpConnectionHandler>");
...
void X2TcpConnectionHandler::Start()
{
	MS_TRACE();
...
void X2TcpConnectionHandler::Write(
  const uint8_t* data1,
  size_t len1,
  const uint8_t* data2,
  size_t len2,
  X2TcpConnectionHandler::onSendCallback* cb)
{
	MS_TRACE();
...
void X2TcpConnectionHandler::ErrorReceiving()
{
	MS_TRACE();
...
bool X2TcpConnectionHandler::SetPeerAddress()
{
	MS_TRACE();
...
inline void X2TcpConnectionHandler::OnUvReadAlloc(size_t /*suggestedSize*/, uv_buf_t* buf)
{
	MS_TRACE();
...
inline void X2TcpConnectionHandler::OnUvRead(ssize_t nread, const uv_buf_t* /*buf*/)
{
	MS_TRACE();
...
inline void X2TcpConnectionHandler::OnUvWrite(int status, X2TcpConnectionHandler::onSendCallback* cb)
{
	MS_TRACE();
