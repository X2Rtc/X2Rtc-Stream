#define MS_CLASS "X2UdpSocketHandler"
...
{
	auto* socket = static_cast<X2UdpSocketHandler*>(handle->data);
...
{
	auto* sendData = static_cast<X2UdpSocketHandler::UvSendData*>(req->data);
...
X2UdpSocketHandler::X2UdpSocketHandler(uv_udp_t* uvHandle) : uvHandle(uvHandle)
{
	MS_TRACE();

	readBuffer = new uint8_t[65536];
...
X2UdpSocketHandler::~X2UdpSocketHandler()
{
	MS_TRACE();
...
void X2UdpSocketHandler::Close()
{
	...
	// Tell the UV handle that the X2UdpSocketHandler has been closed.
	...
	if (err != 0)
		MS_THROW_ERROR("uv_udp_recv_stop() failed: %s", uv_strerror(err));
...
void X2UdpSocketHandler::Dump() const
{
	MS_DUMP("<X2UdpSocketHandler>");
...
void X2UdpSocketHandler::Send(
  const uint8_t* data, size_t len, const struct sockaddr* addr, X2UdpSocketHandler::onSendCallback* cb)
{
	MS_TRACE();
...
bool X2UdpSocketHandler::SetLocalAddress()
{
	MS_TRACE();
...
inline void X2UdpSocketHandler::OnUvRecvAlloc(size_t /*suggestedSize*/, uv_buf_t* buf)
{
	MS_TRACE();
...
inline void X2UdpSocketHandler::OnUvRecv(
...
inline void X2UdpSocketHandler::OnUvSend(int status, X2UdpSocketHandler::onSendCallback* cb)
{
	MS_TRACE();
