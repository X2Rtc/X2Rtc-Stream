#ifndef X2_MS_TCP_CONNECTION_HPP
#define X2_MS_TCP_CONNECTION_HPP

#include "X2common.hpp"
#include <uv.h>
#include <string>

class X2TcpConnectionHandler
{
protected:
...
public:
	...
	public:
		virtual void OnTcpConnectionClosed(X2TcpConnectionHandler* connection) = 0;
	};
...
public:
	...
	struct UvWriteData
	{
		...
		X2TcpConnectionHandler::onSendCallback* cb{ nullptr };
	};

public:
	explicit X2TcpConnectionHandler(size_t bufferSize);
	X2TcpConnectionHandler& operator=(const X2TcpConnectionHandler&) = delete;
	X2TcpConnectionHandler(const X2TcpConnectionHandler&)            = delete;
	virtual ~X2TcpConnectionHandler();
...
public:
	//@Eric - 20230609 - used for uv_loop from user self defined
	void SetSelfUvLoop(uv_loop_t* loop) {
		uv_loop_ = loop;
	}
private:
