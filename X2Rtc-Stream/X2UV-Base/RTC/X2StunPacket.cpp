#define MS_CLASS "RTC::X2StunPacket"
...
namespace x2rtc
{
	...
	const uint8_t X2StunPacket::magicCookie[] = { 0x21, 0x12, 0xA4, 0x42 };

	/* Class methods. */

	X2StunPacket* X2StunPacket::Parse(const uint8_t* data, size_t len)
	{
		MS_TRACE();

		if (!X2StunPacket::IsStun(data, len))
			return nullptr;
		...
		// Get type field.
		const uint16_t msgType = X2Utils::Byte::Get2Bytes(data, 0);

		// Get length field.
		const uint16_t msgLength = X2Utils::Byte::Get2Bytes(data, 2);
		...
		// Create a new X2StunPacket (data + 8 points to the received TransactionID field).
		auto* packet = new X2StunPacket(
		...
		while (pos + 4 <= len)
		{
			...
			auto attrType = static_cast<Attribute>(X2Utils::Byte::Get2Bytes(data, pos));

			// Get the attribute length.
			const uint16_t attrLength = X2Utils::Byte::Get2Bytes(data, pos + 2);
			...
			switch (attrType)
			{
				...
				case Attribute::PRIORITY:
				{
					...
					packet->SetPriority(X2Utils::Byte::Get4Bytes(attrValuePos, 0));
				...
				case Attribute::ICE_CONTROLLING:
				{
					...
					packet->SetIceControlling(X2Utils::Byte::Get8Bytes(attrValuePos, 0));
				...
				case Attribute::ICE_CONTROLLED:
				{
					...
					packet->SetIceControlled(X2Utils::Byte::Get8Bytes(attrValuePos, 0));
				...
				case Attribute::NOMINATION:
				{
					...
					packet->SetNomination(X2Utils::Byte::Get4Bytes(attrValuePos, 0));
				...
				case Attribute::FINGERPRINT:
				{
					...
					fingerprint        = X2Utils::Byte::Get4Bytes(attrValuePos, 0);
				...
				case Attribute::ERROR_CODE:
				{
					...
					const uint8_t errorClass  = X2Utils::Byte::Get1Byte(attrValuePos, 2);
					const uint8_t errorNumber = X2Utils::Byte::Get1Byte(attrValuePos, 3);
			...
			pos =
			  static_cast<size_t>(X2Utils::Byte::PadTo4Bytes(static_cast<uint16_t>(pos + 4 + attrLength)));
		}
		...
		if (hasFingerprint)
		{
			...
			const uint32_t computedFingerprint =
			  X2Utils::Crypto::GetCRC32(data, fingerprintAttrPos) ^ 0x5354554e;
	...
	void X2StunPacket::Dump() const
	{
		MS_TRACE();

		MS_DUMP("<X2StunPacket>");
		...
		if (this->xorMappedAddress != nullptr)
		{
			...
			X2Utils::IP::GetAddressInfo(this->xorMappedAddress, family, ip, port);
	...
	X2StunPacket::Authentication X2StunPacket::CheckAuthentication(
	  const std::string& localUsername, const std::string& localPassword)
	{
		...
		if (this->hasFingerprint)
			...
			X2Utils::Byte::Set2Bytes(this->data, 2, static_cast<uint16_t>(this->size - 20 - 8));

		// Calculate the HMAC-SHA1 of the message according to MESSAGE-INTEGRITY rules.
		const uint8_t* computedMessageIntegrity = X2Utils::Crypto::GetHmacSha1(
		...
		if (this->hasFingerprint)
			X2Utils::Byte::Set2Bytes(this->data, 2, static_cast<uint16_t>(this->size - 20));

		return result;
	}

	X2StunPacket* X2StunPacket::CreateSuccessResponse()
	{
		MS_TRACE();
		...
		return new X2StunPacket(Class::SUCCESS_RESPONSE, this->method, this->transactionId, nullptr, 0);
	}

	X2StunPacket* X2StunPacket::CreateErrorResponse(uint16_t errorCode)
	{
		MS_TRACE();
		...
		auto* response =
		  new X2StunPacket(Class::ERROR_RESPONSE, this->method, this->transactionId, nullptr, 0);
	...
	void X2StunPacket::Authenticate(const std::string& password)
	{
	...
	void X2StunPacket::Serialize(uint8_t* buffer)
	{
		MS_TRACE();
		...
		bool addXorMappedAddress =
		  ((this->xorMappedAddress != nullptr) && this->method == X2StunPacket::Method::BINDING &&
		...
		if (!this->username.empty())
		{
			usernamePaddedLen = X2Utils::Byte::PadTo4Bytes(static_cast<uint16_t>(this->username.length()));
		...
		// Set type field.
		X2Utils::Byte::Set2Bytes(buffer, 0, typeField);
		// Set length field.
		X2Utils::Byte::Set2Bytes(buffer, 2, static_cast<uint16_t>(this->size) - 20);
		// Set magic cookie.
		std::memcpy(buffer + 4, X2StunPacket::magicCookie, 4);
		...
		{
			X2Utils::Byte::Set2Bytes(buffer, pos, static_cast<uint16_t>(Attribute::USERNAME));
			X2Utils::Byte::Set2Bytes(buffer, pos + 2, static_cast<uint16_t>(this->username.length()));
		...
		if (this->priority != 0u)
		{
			X2Utils::Byte::Set2Bytes(buffer, pos, static_cast<uint16_t>(Attribute::PRIORITY));
			X2Utils::Byte::Set2Bytes(buffer, pos + 2, 4);
			X2Utils::Byte::Set4Bytes(buffer, pos + 4, this->priority);
			pos += 4 + 4;
		}
		...
		if (this->iceControlling != 0u)
		{
			X2Utils::Byte::Set2Bytes(buffer, pos, static_cast<uint16_t>(Attribute::ICE_CONTROLLING));
			X2Utils::Byte::Set2Bytes(buffer, pos + 2, 8);
			X2Utils::Byte::Set8Bytes(buffer, pos + 4, this->iceControlling);
			pos += 4 + 8;
		}
		...
		if (this->iceControlled != 0u)
		{
			X2Utils::Byte::Set2Bytes(buffer, pos, static_cast<uint16_t>(Attribute::ICE_CONTROLLED));
			X2Utils::Byte::Set2Bytes(buffer, pos + 2, 8);
			X2Utils::Byte::Set8Bytes(buffer, pos + 4, this->iceControlled);
			pos += 4 + 8;
		}
		...
		if (this->hasUseCandidate)
		{
			X2Utils::Byte::Set2Bytes(buffer, pos, static_cast<uint16_t>(Attribute::USE_CANDIDATE));
			X2Utils::Byte::Set2Bytes(buffer, pos + 2, 0);
			pos += 4;
		}
		...
		if (addXorMappedAddress)
		{
			X2Utils::Byte::Set2Bytes(buffer, pos, static_cast<uint16_t>(Attribute::XOR_MAPPED_ADDRESS));
			X2Utils::Byte::Set2Bytes(buffer, pos + 2, xorMappedAddressPaddedLen);
			...
			switch (this->xorMappedAddress->sa_family)
			{
				case AF_INET:
				{
					...
					std::memcpy(
					  ...
					  2);
					attrValue[2] ^= X2StunPacket::magicCookie[0];
					attrValue[3] ^= X2StunPacket::magicCookie[1];
					// Set address and XOR it.
					std::memcpy(
					  attrValue + 4,
					  &(reinterpret_cast<const sockaddr_in*>(this->xorMappedAddress))->sin_addr.s_addr,
					  4);
					attrValue[4] ^= X2StunPacket::magicCookie[0];
					attrValue[5] ^= X2StunPacket::magicCookie[1];
					attrValue[6] ^= X2StunPacket::magicCookie[2];
					attrValue[7] ^= X2StunPacket::magicCookie[3];
		...
		if (addErrorCode)
		{
			X2Utils::Byte::Set2Bytes(buffer, pos, static_cast<uint16_t>(Attribute::ERROR_CODE));
			X2Utils::Byte::Set2Bytes(buffer, pos + 2, 4);
			...
			X2Utils::Byte::Set2Bytes(buffer, pos + 4, 0);
			X2Utils::Byte::Set1Byte(buffer, pos + 6, codeClass);
			X2Utils::Byte::Set1Byte(buffer, pos + 7, codeNumber);
			pos += 4 + 4;
		}
		...
		if (addMessageIntegrity)
		{
			// Ignore FINGERPRINT.
			if (addFingerprint)
				X2Utils::Byte::Set2Bytes(buffer, 2, static_cast<uint16_t>(this->size - 20 - 8));

			// Calculate the HMAC-SHA1 of the packet according to MESSAGE-INTEGRITY rules.
			const uint8_t* computedMessageIntegrity =
			  X2Utils::Crypto::GetHmacSha1(this->password, buffer, pos);

			X2Utils::Byte::Set2Bytes(buffer, pos, static_cast<uint16_t>(Attribute::MESSAGE_INTEGRITY));
			X2Utils::Byte::Set2Bytes(buffer, pos + 2, 20);
		...
		if (addFingerprint)
		{
			...
			const uint32_t computedFingerprint = X2Utils::Crypto::GetCRC32(buffer, pos) ^ 0x5354554e;

			X2Utils::Byte::Set2Bytes(buffer, pos, static_cast<uint16_t>(Attribute::FINGERPRINT));
			X2Utils::Byte::Set2Bytes(buffer, pos + 2, 4);
			X2Utils::Byte::Set4Bytes(buffer, pos + 4, computedFingerprint);
			pos += 4 + 4;
