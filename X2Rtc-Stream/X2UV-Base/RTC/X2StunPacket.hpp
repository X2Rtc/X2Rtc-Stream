#ifndef X2_MS_RTC_STUN_PACKET_HPP
#define X2_MS_RTC_STUN_PACKET_HPP
...
namespace x2rtc
{
	...
	public:
		static bool IsStun(const uint8_t* data, size_t len)
		{
			...
			return (
				...
				(data[4] == X2StunPacket::magicCookie[0]) && (data[5] == X2StunPacket::magicCookie[1]) &&
				(data[6] == X2StunPacket::magicCookie[2]) && (data[7] == X2StunPacket::magicCookie[3])
			);
			// clang-format on
		}
		static X2StunPacket* Parse(const uint8_t* data, size_t len);
	...
	public:
		...
		X2StunPacket* CreateSuccessResponse();
		X2StunPacket* CreateErrorResponse(uint16_t errorCode);
