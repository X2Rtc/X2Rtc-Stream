...
namespace X2Utils
{
	...
	public:
		...
		//@Eric - Add for Tcp rpc connection
		static void Set4LEBytes(uint8_t* data, size_t i, uint32_t value)
		{
		...
		static uint32_t Get4LEBytes(const uint8_t* data, size_t i)
		{
			return uint32_t{ data[i + 3] } << 24 | uint32_t{ data[i + 2] } << 16 |
				uint32_t{ data[i + 1] } << 8 | uint32_t{ data[i] };
		}
	};
