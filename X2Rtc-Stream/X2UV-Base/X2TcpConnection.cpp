#define MS_CLASS "RTC::X2TcpConnection"
// #define MS_LOG_DEV_LEVEL 3

#include "X2TcpConnection.hpp"
...
namespace x2rtc
{
	...
	X2TcpConnection::X2TcpConnection(Listener* listener, size_t bufferSize)
	  : ::X2TcpConnectionHandler::X2TcpConnectionHandler(bufferSize), listener(listener)
	{
		MS_TRACE();
	}

	X2TcpConnection::~X2TcpConnection()
	{
		MS_TRACE();
	}

	void X2TcpConnection::UserOnTcpConnectionRead()
	{
		MS_TRACE();
		...
		while (true)
		{
			...
			if (dataLen >= 2)
				packetLen = size_t{ X2Utils::Byte::Get2Bytes(this->buffer + this->frameStart, 0) };
	...
	void X2TcpConnection::Send(const uint8_t* data, size_t len, ::X2TcpConnectionHandler::onSendCallback* cb)
	{
		MS_TRACE();
		...
		X2Utils::Byte::Set2Bytes(frameLen, 0, len);
		::X2TcpConnectionHandler::Write(frameLen, 2, data, len, cb);
	}
} // namespace RTC
