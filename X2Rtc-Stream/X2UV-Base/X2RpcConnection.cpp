#define MS_CLASS "RTC::X2RpcConnection"
...
namespace x2rtc
{
	...
	X2RpcConnection::X2RpcConnection(Listener* listener, size_t bufferSize)
	  : ::X2TcpConnectionHandler::X2TcpConnectionHandler(bufferSize), listener(listener)
	{
		MS_TRACE();
	}

	X2RpcConnection::~X2RpcConnection()
	{
		MS_TRACE();
	}

	void X2RpcConnection::UserOnTcpConnectionRead()
	{
		MS_TRACE();
		...
		while (true)
		{
			...
			if (dataLen >= 4)
				packetLen = size_t{ X2Utils::Byte::Get4LEBytes(this->buffer + this->frameStart, 0) };

			// We have packetLen bytes.
			if (dataLen >= 4 && dataLen >= 4 + packetLen)
			{
				const uint8_t* packet = this->buffer + this->frameStart + 4;
				...
				if ((this->frameStart + 4 + packetLen) == this->bufferSize)
				{
	...
	void X2RpcConnection::Send(const uint8_t* data, size_t len, ::X2TcpConnectionHandler::onSendCallback* cb)
	{
		MS_TRACE();
		...
		X2Utils::Byte::Set4LEBytes(frameLen, 0, len);
		::X2TcpConnectionHandler::Write(frameLen, 4, data, len, cb);
	}
} // namespace RTC
